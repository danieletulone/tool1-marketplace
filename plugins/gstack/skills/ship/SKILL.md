...
1. Find and read the review checklist from the gstack plugin. Search for it:
```bash
for candidate in \
  "$(pwd)/plugins/gstack/skills/review/checklist.md" \
  "$(pwd)/.claude/plugins/gstack/skills/review/checklist.md" \
  "$HOME/.claude/plugins/gstack/skills/review/checklist.md"; do
  if [ -f "$candidate" ]; then echo "FOUND:$candidate"; break; fi
done
```
Read the file at the found path. If the file cannot be read, **STOP** and report the error.
